...
function mountStyles() {
  ...
  link.rel = "stylesheet";
  link.href = "./src/ownerReportGrantsV9.css?v=20260812-2";
...
async function createOwnerGrant(reportId, granteeUid, reason) {
  ...
  const [report, grantee, grant] = await Promise.all([
    readDoc("reports", reportId),
    readDoc("users", granteeUid),
    readDoc("reportAccessGrants", reportId).catch(() => null)
  ]);
  ...
  approved.add(granteeUid);
  approved.add(ownerMarker(granteeUid));
  if (approved.size > 100) throw new Error("This report has reached its access-grant limit.");

  await Fire.setDoc(Fire.doc(db, "reportAccessGrants", report.id), {
    reportId: report.id,
...
async function revokeOwnerGrant(reportId, granteeUid) {
  ...
  const [grant, request] = await Promise.all([
    readDoc("reportAccessGrants", reportId),
    readDoc("reportAccessRequests", `${reportId}__${granteeUid}`).catch(() => null)
  ]);
  if (!grant) throw new Error("The grant record no longer exists.");
  const approved = new Set(Array.isArray(grant.approvedUids) ? grant.approvedUids : []);
  approved.delete(ownerMarker(granteeUid));
  if (request?.status !== "approved") approved.delete(granteeUid);

  if (approved.size) {
    await Fire.updateDoc(Fire.doc(db, "reportAccessGrants", reportId), {
      approvedUids: [...approved],
      updatedAt: Fire.serverTimestamp()
    });
  } else {
    await Fire.deleteDoc(Fire.doc(db, "reportAccessGrants", reportId));
  }
...
async function mountSubjectVisibility() {
  ...
  const grants = await readWhere("reportAccessGrants", "subjectProfileId", "==", authUser.uid).catch(() => []);
  const directRows = [];
  for (const grant of grants) for (const uid of directGrantees(grant)) directRows.push({ grant, uid });
  ...
  panel.innerHTML = `<div class="panel-header"><div><p class="eyebrow">Owner-authorized Access</p><h2>Cognitus Owner grants</h2></div><span>${directRows.length} active</span></div><p class="v8-section-intro">These permissions were issued directly by a Cognitus Owner and are shown here so access to your reports is never invisible.</p><div class="v9-owner-grant-list">${directRows.map((row) => { const user = usersById.get(row.uid); return `<article><div><strong>${escapeHtml(user?.displayName || user?.cognitusId || row.uid)}</strong><span>${escapeHtml(row.grant.reportId || row.grant.id)}</span><small>Direct Cognitus Owner authorization · ${escapeHtml(formatTimestamp(row.grant.updatedAt))}</small></div><span class="v9-owner-badge">Owner Granted</span></article>`; }).join("")}</div>`;
  if (route() === "/reports") {
...
async function mountFullReportProof() {
  ...
  const grant = await readDoc("reportAccessGrants", reportId).catch(() => null);
  if (!grant || !(grant.approvedUids || []).includes(ownerMarker(authUser.uid))) return;
  ...
  proof.innerHTML = `<span>Granted by Cognitus Owner</span><strong>Cognitus Owner directly authorized this account to view the complete report.</strong><small>This permission does not require a separate subject-approval request.</small>`;
