...
async function adminPage() {
  ...
  root.innerHTML = `<section class="hero hero-wide"><p class="eyebrow">Administration</p><h1>Control access and verification.</h1><p>Main Cognitus roles now use explicit capabilities instead of rank-style inheritance. Reviewers do not automatically inherit organization-administrator authority. Admins may manage non-owner accounts; only Owners may grant or remove Owner access.</p></section><section class="admin-tabs"><button class="button button-dark" data-tab="users" type="button">Users</button><button class="button button-light" data-tab="orgs" type="button">Organizations</button><button class="button button-light" data-tab="activity" type="button">Activity</button></section><section id="admin-users" class="panel">${renderUsersTable(users)}</section><section id="admin-orgs" class="panel" hidden>${renderOrganizationsAdmin(organizations)}</section><section id="admin-activity" class="panel" hidden>${renderActivity(activity)}</section>`;
...
async function discordAccessPage() {
  ...
  const renderPayload = (data) => {
    ...
    const talent = cognitus?.talent || null;
