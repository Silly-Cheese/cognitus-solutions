...
const checks = [
  ...
  [report.includes('readWhere("screeningReportSummaries", "subjectProfileId", "==", profileId)'), "person dossiers load reviewed screening indexes with a single equality query"],
  [report.includes('readDoc("reports", summary.reportId || summary.id)') && report.includes("fullAccess"), "full narratives are included only when Firestore authorizes direct report access"],
